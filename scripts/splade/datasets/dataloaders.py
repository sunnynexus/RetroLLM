...
class TextAnswerCollectionDataLoader(DataLoaderWrapper):
    """与之前相同，但还返回输入文本和答案。"""

    def collate_fn(self, batch):
        """
        batch 是一个元组的列表，每个元组有3个元素 (id_, doc, answer)
        """
        id_, d, a = zip(*batch)
        processed_passage = self.tokenizer(
            list(d),
            add_special_tokens=True,
            padding="longest",  # pad 到批次中的最长序列长度
            truncation="longest_first",  # 截断到 self.max_length
            max_length=self.max_length,
            return_attention_mask=True
        )
        return {
