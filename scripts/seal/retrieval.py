...
def batch_generate_keys(searcher, queries, constrained_generation=True):
    ...
    def process_batch(inputs):
        ...
        print("---\nbatch inputs:", inputs)
        # print('searcher:', searcher.decode_body, searcher.add_query_to_keys, searcher.decode_titles, searcher.decode_code, searcher.rescore, searcher.use_markers, searcher.unigram_scores)
        ...
        if searcher.decode_body:
            ...
            for fk in found_keys:
                decoded_texts = [searcher.bart_tokenizer.decode(ids, skip_special_tokens=True) for _, ids in fk]
                print("Decoded texts:", decoded_texts)

            # batch inputs: [' who sings does he love me with reba', ' where do the great lakes meet the ocean']
            # Decoded texts: [' "', ' song', ' Hot', ' single', ' "Does', ' song was', ' song)', ' song recorded', ' "Does He', ' song was released', ' song was written', ' song was a', ' "Does He Love', ' song was released as', ' song was released in', ' song was released on', ' "Does He Love Me', ' song was released as the', ' song was released as a', ' "Does He Love You', ' "Does He Love Me"', ' "Does He Love Me?"', ' "Does He Love Me?",', ' song was released as the second', ' "Does He Love Me" went', ' "Does He Love Me" won', ' "Does He Love Me?" ft', ' "Does He Love Me" won the', ' "Does He Love Me" went No', ' "Does He Love Me" won the']
            # Decoded texts: [' Great', ' Lake', ' Lakes', ' lakes', ' Great Lakes', ' Lake Superior', ' Lake Michigan', ' Lake Ontario', ' Great Lakes basin', ' Great Lakes Basin', ' Great Lakes drainage', ' Great Lakes is', ' Great Lakes basin is', ' Great Lakes basin,', ' Great Lakes Basin,', ' Great Lakes Basin is', ' Great Lakes basin, and', ' Great Lakes basin, with', ' Great Lakes basin is from', ' Great Lakes basin, and many', ' Great Lakes basin, with Kingston', ' Great Lakes basin, and many of', ' Great Lakes basin, with Kingston,', ' Great Lakes basin, and many of these', ' Great Lakes basin, with Kingston, Ontario', ' Great Lakes basin, and many of these']
        ...
        if searcher.decode_titles:
            ...
            for fk in decoded_title:
                decoded_texts = [searcher.bart_tokenizer.decode(ids, skip_special_tokens=True) for _, ids in fk]
                print("Decoded title:", decoded_texts)
    ...
    def batch_search(self, queries, k: int = 10, added_documents=None, detokenize=None) -> List[List[SEALDocument]]:
        ...
        for query, res in zip(queries, results):
            ...
            for idx, (score, kk, _, full, _) in islice(res.items(), k):
                ...
                # print(f"Generated Text for DocID {idx}: {doc.raw_text()}")
                # os._exit(0)
                if self.include_keys:
                    for _k, _ in kk:
                        if _k not in keys:
                            keys[_k] = (self.bart_tokenizer.decode(list(_k), clean_up_tokenization_spaces=False), self.fm_index.get_count(list(_k)))
                    kk = [(*keys[_k], s) for _k, s in kk]
                    doc.keys = kk
