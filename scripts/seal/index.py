...
class FMIndex(_FMIndex):
    ...
    def get_range_from_doc(self, sequence: List[int], doc_index: Optional[int] = None) -> Tuple[int, int]:
        """
        Finds the FM-index rows that match the input prefix `sequence`.
        Optionally restricts search to a specific document.
        """
        start_row = 0
        end_row = self.size()
        if doc_index is not None:
            start_row = self.beginnings[doc_index]
            end_row = self.beginnings[doc_index + 1]

        for token in sequence:
            print("start_row", start_row, "end_row", end_row)
            start_row, end_row = self.backward_search_step(token + SHIFT, start_row, end_row)
            # Constrain the search within the document boundaries
            if doc_index is not None:
                start_row = max(start_row, self.beginnings[doc_index])
                end_row = min(end_row, self.beginnings[doc_index + 1])
            if start_row >= end_row:
                break

        print("start_row", start_row, "end_row", end_row)
        end_row = min(end_row, end_row + 1)  # Ensure we do not exceed the document boundary
        return start_row, end_row

    def get_doc_indices_from_doc(self, sequence: List[int], doc_index: Optional[int] = None) -> Iterator[int]:
        """
        Finds the documents that contain the input prefix `sequence`.
        Optionally restricts search to a specific document.
        """
        start, end = self.get_range(sequence, doc_index)
        if doc_index is not None:
            # Return only the specified document index if the range is valid
            if start < end:
                yield doc_index
        else:
